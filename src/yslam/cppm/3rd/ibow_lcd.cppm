export module ibow_lcd;
export import std;
export import cv;
import obindex2;

export namespace ibow_lcd {
// 1. Island
struct Island {
    ...
    std::vector<obindex2::ImageMatch>* image_matches_filt)
{
    ...
    for (unsigned i = 0; i < image_matches.size(); i++) {
        ...
        double new_score = (image_matches[i].score - min_score) / (max_score - min_score);
...
unsigned LCDetector::checkEpipolarGeometry(
    ...
    const std::vector<cv::Point2f>& train)
{
    std::vector<cv::uchar> inliers(query.size(), 0);
    if (query.size() > 7) {
