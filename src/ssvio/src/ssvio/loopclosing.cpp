...
bool LoopClosing::DetectLoop()
{
    ...
    for (const auto& db : key_frame_database_) {
        ...
        if (!current_keyframe_->bow2_vec_.empty() && !db.second->bow2_vec_.empty()) {
            float similarity_score = dbow2_vocabulary_->score(current_keyframe_->bow2_vec_, db.second->bow2_vec_);
...
bool LoopClosing::MatchFeatures()
{
    ...
    for (const auto& mt : matches) {
        if (mt.distance <= std::max(2 * min_distance, 30.0)) {
            ...
            int current_feature_id = current_keyframe_->pyramid_key_points_[mt.trainIdx].class_id;
...
void LoopClosing::CorrectActivateKeyframeAndMappoint()
{
    ...
    for (auto iter = set_valid_feature_matches_.begin();
        ...
        auto current_mp = current_keyframe_->features_left_[current_feature_id]->map_point_.lock();
...
void LoopClosing::ProcessNewKeyframe()
{
    ...
    std::vector<cv::Mat> desc = ConvertToDescriptorVector(current_keyframe_->ORBDescriptors_);
...
{
    std::unique_lock<std::mutex> lck(all_new_keyframe_mutex_);
    if (last_closed_keyframe_ == nullptr || new_kf->key_frame_id_ - last_closed_keyframe_->key_frame_id_ > 5) {
...
void LoopClosing::LoadParam()
{
    ...
    keyframe_database_min_size_ = Setting::Get<int>("Loop.Closig.Keyframe.Database.Min.Size");
