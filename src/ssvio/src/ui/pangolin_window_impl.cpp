...
bool PangolinWindowImpl::InitPangolin()
{
    ...
    /// create trajectory
    no_backend_vo_traj_ = std::make_unique<ui::TrajectoryUI>(Eigen::Vector3f(0.0, 1.0, 0.0)); ///
    have_backend_kf_vo_traj_ = std::make_unique<ui::TrajectoryUI>(Eigen::Vector3f(0, 1.0, 0)); /// yellow
...
void PangolinWindowImpl::CreateDisplayLayout()
{
    ...
    plotter_yam_angle_ = std::make_unique<pangolin::Plotter>(&log_yaw_angle_, -10, 100, -M_PI, M_PI, 75, 2);
...
{
    std::string save_file_path = ssvio::Setting::Get<std::string>("Trajectory.Save.Path");
