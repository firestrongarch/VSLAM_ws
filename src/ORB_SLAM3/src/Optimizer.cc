...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    std::vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPose *> vpEdgesMono;           // 存放单目边
    std::vector<ORB_SLAM3::EdgeSE3ProjectXYZOnlyPoseToBody *> vpEdgesMono_FHR; // 存放另一目的边
    std::vector<size_t> vnIndexEdgeMono, vnIndexEdgeRight;                     // 边对应特征点的id
    vpEdgesMono.reserve(N);
    vpEdgesMono_FHR.reserve(N);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeRight.reserve(N);

    std::vector<g2o::EdgeStereoSE3ProjectXYZOnlyPose *> vpEdgesStereo; // 存放双目边
...
void Optimizer::BundleAdjustment(
    const std::vector<KeyFrame *> &vpKFs, const std::vector<MapPoint *> &vpMP,
    ...
    Eigen::MatrixXd &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    ...
    int its = 200;
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const std::vector<KeyFrame *> vpKFs = pMap->GetAllKeyFrames();  // 获取所有关键帧
    ...
    // 6. imu信息链接重力方向与尺度信息
    std::vector<EdgeInertialGS *> vpei;  // 后面虽然加入了边，但是没有用到，应该调试用的
...
int Optimizer::OptimizeSim3(
    KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2,
    const bool bFixScale, Eigen::Matrix<double, 7, 7> &mAcumHessian, const bool bAllPoints)
{
    ...
    std::vector<ORB_SLAM3::EdgeSim3ProjectXYZ *> vpEdges12;         // pKF2对应的MapPoints到pKF1的投影
    std::vector<ORB_SLAM3::EdgeInverseSim3ProjectXYZ *> vpEdges21;  // pKF1对应的MapPoints到pKF2的投影
    ...
    const std::map<KeyFrame *, std::set<KeyFrame *>> &LoopConnections, const bool &bFixScale)
{
    ...
    std::vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vScw(nMaxKFid + 1); // 存放每一帧优化前的sim3
    // 记录所有关键帧经过本次本质图优化过的位姿
    std::vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vCorrectedSwc(nMaxKFid + 1); // 存放每一帧优化后的sim3，修正mp位姿用
    // 这个变量没有用
    std::vector<g2o::VertexSim3Expmap *> vpVertices(nMaxKFid + 1); // 存放节点，没用，还占地方
...
void Optimizer::LocalBundleAdjustment(
    KeyFrame *pMainKF, std::vector<KeyFrame *> vpAdjustKF, std::vector<KeyFrame *> vpFixedKF, bool *pbStopFlag)
{
    ...
    long unsigned int maxKFid = 0;
    std::set<KeyFrame *> spKeyFrameBA;  // 存放关键帧，包含固定的与不固定的
    ...
    // Set MapPoint vertices
    std::map<KeyFrame *, int> mpObsKFs;      // 统计每个关键帧对应的MP点数，调试输出用
    std::map<KeyFrame *, int> mpObsFinalKFs; // 统计每个MP对应的关键帧数，调试输出用
    std::map<MapPoint *, int> mpObsMPs;      // 统计每个MP被观测的图片数，双目就是两个，调试输出用
    ...
    for (KeyFrame *pKFi : vpAdjustKF)
    {
        ...
        int numMonoBadPoints = 0, numMonoOptPoints = 0;
        int numStereoBadPoints = 0, numStereoOptPoints = 0;
        std::vector<MapPoint *> vpMonoMPsOpt, vpStereoMPsOpt; // 存放mp内点
        std::vector<MapPoint *> vpMonoMPsBad, vpStereoMPsBad; // 存放mp外点
 ...
 */
void Optimizer::OptimizeEssentialGraph(
    KeyFrame *pCurKF, std::vector<KeyFrame *> &vpFixedKFs, std::vector<KeyFrame *> &vpFixedCorrectedKFs,
    std::vector<KeyFrame *> &vpNonFixedKFs, std::vector<MapPoint *> &vpNonCorrectedMPs)
{
    ...
    std::vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vScw(nMaxKFid + 1);          // 存放每一帧优化前的sim3
    std::vector<g2o::Sim3, Eigen::aligned_allocator<g2o::Sim3>> vCorrectedSwc(nMaxKFid + 1); // 存放每一帧优化后的sim3，调试输出用，没有实际作用
    std::vector<g2o::VertexSim3Expmap *> vpVertices(nMaxKFid + 1);                           // 存放节点，没用，还占地方
