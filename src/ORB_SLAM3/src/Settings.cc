...
template <>
std::string Settings::readParameter<string>(cv::FileStorage &fSettings, const std::string &name, bool &found, const bool required)
{
    cv::FileNode node = fSettings[name];
    if (node.empty())
    {
...
void Settings::readCamera1(cv::FileStorage &fSettings)
{
    ...
    // Read camera model
    std::string cameraModel = readParameter<string>(fSettings, "Camera.type", found);
